...
% Last Modified by GUIDE v2.5 11-Jul-2013 15:23:27
...
if findex~=0 
    ...
    if ~isempty(strfind(fpath, '.txt')) || ~isempty(strfind(fpath, '.csv'))
        ...
        if isstruct(data)
            spec = data.data;
            wave = cellfun(@(x) str2couble(x),data.textdata(1,2:end));
            label = data.textdata(1,2:end);
...
dialog_sgder2(wave, label, spec, 'table_main', handles.table_main);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global label spec wave
mwsnvGUI(label, spec ,wave);
